...
test('preserves current Logseq row order and excludes full-block renderers', () => {
  ...
  assert.match(host_pr_parity_style, /--ls-bidi-block-children-indent: 7px;/)
  assert.match(host_pr_parity_style, /margin-inline-end: var\(--ls-bidi-block-children-indent, 29px\);/)
...
test('editor override css uses safe block selectors for both directions', () => {
  ...
  assert.match(ltr_css, /margin-left: var\(--ls-bidi-block-children-indent, 29px\);/)
